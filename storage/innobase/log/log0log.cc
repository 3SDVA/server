...
*/
struct Log_write_completion_info
{
  tpool::aiocb aio_cb;
  lsn_t write_lsn=0;
  bool do_flush=false;
...
#endif
  Log_write_completion_info()
  {
    aio_cb.m_callback= (tpool::callback_func)log_write_complete_io;
  }
};

static Log_write_completion_info completion_info;
...
{
  lsn_t write_lsn=0;
  DBUG_ASSERT(completion_info.do_write || completion_info.do_flush);

  if (completion_info.do_write)
  {
    log_sys.log.complete_write(cb);
    write_lsn = completion_info.write_lsn;
    DBUG_ASSERT(write_lock.value() < write_lsn);
    DBUG_ASSERT(log_sys.write_lsn < write_lsn);
    DBUG_ASSERT(log_sys.get_flushed_lsn() < write_lsn);
    DBUG_ASSERT(log_sys.get_flushed_lsn() <= log_sys.write_lsn);
    log_sys.write_lsn= write_lsn;
    write_lock.release(write_lsn);
  }


  if(completion_info.do_flush)
  {
    /* Flush the highest written lsn.*/
    auto flush_lsn= write_lock.value();
    DBUG_ASSERT(write_lsn <= flush_lsn);

    DBUG_ASSERT(flush_lock.value() < flush_lsn);
    flush_lock.set_pending(flush_lsn);
    log_write_flush_to_disk_low(flush_lsn);
    flush_lock.release(flush_lsn);
#ifndef DBUG_OFF
    if (completion_info.crash_on_completion)
      DBUG_SUICIDE();
#endif
                     ...
                     const completion_callback *callback)
{
  ...
  if (flush_to_disk)
  {
    ...
    */
    do_write= write_lock.acquire(lsn, nullptr) == group_commit_lock::ACQUIRED;
  }
  ...
  DBUG_EXECUTE_IF("crash_after_log_write_upto",
                  completion_info.crash_on_completion= true;);

  completion_info.do_flush= do_flush;
  completion_info.do_write= do_write;
  ...
  if (do_write)
  {
    ...
    completion_info.write_lsn= write_lsn;

    log_write(rotate_key, use_synchronous_io, &completion_info.aio_cb);
    /* Releasing locks and flushing etc will be done by completion routine of the AIO.*/
    return;
  }
  /* Only flush*/
  if (use_synchronous_io)
    log_write_complete_io(&completion_info.aio_cb);
  else
  {
    static tpool::task complete_io_task(
        (tpool::callback_func) log_write_complete_io, nullptr);
    srv_thread_pool->submit_task(&complete_io_task);
  }
}
